...
class DbClassOrder implements DbClass {
	...
	private void submitOrderItem(OrderItem item) throws DatabaseException {
        ...
        //implement
        submitOrderItemParams = new Object[]{item.getOrderId(),item.getProductId(),item.getQuantity(),item.getTotalPrice(),0.0,0.0};
        submitOrderItemTypes = new int[]{Types.SMALLINT,Types.SMALLINT,Types.INTEGER,Types.DOUBLE,Types.DOUBLE,Types.DOUBLE};
    ...
    private void populateOrderItems(ResultSet rs) throws DatabaseException {
    	List<OrderItem> lst = new ArrayList<>();
    	 try {
             while(rs.next()){
            	  OrderItemImpl orderItemData = new OrderItemImpl(
            			  rs.getString("productname").trim(),
            			  Integer.parseInt(rs.getString("quantity").trim()),
            			  Double.parseDouble(rs.getString("priceperunit").trim()));
    ...
    private void populateOrderData(ResultSet resultSet) throws DatabaseException { 
         ...
         try {
             while(resultSet.next()){
            	LocalDate date = LocalDate.parse(resultSet.getString("orderdate").trim(), formatter);
